...
GlobalDescriptorTable::SegmentDescriptor::SegmentDescriptor(uint32_t base,uint32_t limit,uint8_t type)
{
    ...
    if(limit<=65536)
    {
        target[6]=0x40;  //16bit-mode
    }
    else    //squeeze 32bit to 20bit,cut off 12bit of right(it is legal only if they all 1)
    {       //so if there are some 0,we need to do some dispose.
        if((limit&0xFFF)!=0xFFF)
            limit=(limit>>12)-1;
        else 
            limit=limit>>12;
        target[6] = 0xC0;//32bit-mode
    }
    //encode the limit
    target[0]=limit&0xFF;
